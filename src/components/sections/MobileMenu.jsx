...
export const MobileMenu = ({menuOpen, setMenuOpen}) =>{
    // handles moblie menu --> when screen size is small all navbar become vertical with 3 stripes to open navbar and x to exit
    return (
        <div className={`fixed top-0 left-0 w-full bg-gradient-to-b from-orange-700 to-orange-300 z-40 flex flex-col items-center justify-center
            transition-all duration-300 ease-in-out

            ${menuOpen ? "h-screen opacity-100 pointer-events-auto" : "h-0 opacity-0 pointer-events-none"}
        `}>
            {/* keeps all different options centered and blurred background */}
            ...
            {/* annimations and setMenuOpen to false to close the navbar when section is selected */}
